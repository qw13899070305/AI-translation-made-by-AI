...
ALL_CONFIG_PARAMS = [
    ...
    ("use_fp8", False, "启用动态 FP8 训练" if LANG=="zh" else "Use dynamic FP8 training"),
    ("use_hta", False, "启用 HTA 学习率调度" if LANG=="zh" else "Use HTA learning rate schedule"),
    ("hta_warmup_steps", 100, "HTA warmup 步数" if LANG=="zh" else "HTA warmup steps"),
    ("use_muon", False, "使用 Muon 优化器" if LANG=="zh" else "Use Muon optimizer"),
    ("use_muon_clip", False, "使用 MuonClip 梯度裁剪" if LANG=="zh" else "Use MuonClip gradient clipping"),
    ("muon_clip_grad", 1.0, "MuonClip 梯度裁剪阈值" if LANG=="zh" else "MuonClip grad clip threshold"),
    ("muon_clip_update", 1.0, "MuonClip 更新量裁剪阈值" if LANG=="zh" else "MuonClip update clip threshold"),
